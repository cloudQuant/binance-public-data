#!/usr/bin/env python3
"""
Binance COIN-M Futures Daily Index Price Klines Downloader

Optimized script for downloading daily index price klines history from Binance COIN-M Futures.
This script downloads DAILY index price klines data.
...
Examples:
    ...
    python3 scripts/download-futures-cm-daily-indexPriceKlines.py -s BTCUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with custom intervals
    python3 scripts/download-futures-cm-daily-indexPriceKlines.py -s BTCUSD_PERP -i 1h 4h -startDate 2023-01-01 -endDate 2023-12-31

    # Download multiple symbols
    python3 scripts/download-futures-cm-daily-indexPriceKlines.py -s BTCUSD_PERP ETHUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with auto-detected symbols (all cm symbols)
    python3 scripts/download-futures-cm-daily-indexPriceKlines.py -startDate 2023-01-01 -endDate 2023-12-31

    # Specify custom output folder
    python3 scripts/download-futures-cm-daily-indexPriceKlines.py -s BTCUSD_PERP -folder /data/binance -startDate 2023-01-01 -endDate 2023-12-31
"""
...
def download_index_price_klines_daily(
    symbols: list,
    intervals: list,
    start_date: str,
    end_date: str,
    ...
    """
    Download COIN-M Futures daily index price klines data.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Daily Index Price Klines Downloader\n'
                    'Downloads DAILY index price klines data for COIN-M Futures.',
    ...
    logger.info("Binance COIN-M Futures Daily Index Price Klines Downloader")
    logger.info(f"Market type: cm (COIN-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: DAILY index price klines")
    logger.info(f"Intervals: {', '.join(args.intervals)}")

    return download_index_price_klines_daily(
        symbols=symbols,
