#!/usr/bin/env python3
"""
Binance COIN-M Futures Daily Liquidation Snapshot Downloader

Optimized script for downloading daily liquidation snapshot history from Binance COIN-M Futures.
This script downloads DAILY liquidation snapshot data.

Features:
- Hardcoded for COIN-M Futures (cm) market
- Downloads DAILY data only (monthly not available for liquidation snapshots)
- Auto-skips existing files
- Shows detailed progress (symbol, date, file size)
- Uses 10 concurrent threads for faster downloads

Examples:
    # Download for specific symbol and date range
    python3 scripts/download-futures-cm-daily-liquidationSnapshot.py -s ADAUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download multiple symbols
    python3 scripts/download-futures-cm-daily-liquidationSnapshot.py -s ADAUSD_PERP BTCUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with auto-detected symbols (all cm symbols)
    python3 scripts/download-futures-cm-daily-liquidationSnapshot.py -startDate 2023-01-01 -endDate 2023-12-31

    # Specify custom output folder
    python3 scripts/download-futures-cm-daily-liquidationSnapshot.py -s ADAUSD_PERP -folder /data/binance -startDate 2023-01-01 -endDate 2023-12-31
"""
...
from binance_data_downloader.downloaders.liquidation_snapshot_downloader import LiquidationSnapshotDownloader
from binance_data_downloader.utils.logger_setup import setup_logger, log_level_from_string


def download_liquidation_snapshot_daily(
    symbols: list,
    start_date: str,
    end_date: str,
    folder: str = None,
    ...
    """
    Download COIN-M Futures daily liquidation snapshot data.
    ...
    try:
        ...
        downloader = LiquidationSnapshotDownloader(
            trading_type='cm',
            max_workers=10
        )
        ...
        for symbol in symbols:
            ...
            downloaded_count = downloader.download_daily(
                symbols=[symbol],
                intervals=None,  # Liquidation snapshots don't support intervals
                dates=date_list,
                folder=folder,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Daily Liquidation Snapshot Downloader\n'
                    'Downloads DAILY liquidation snapshot data for COIN-M Futures.',
    ...
    parser.add_argument(
        ...
        nargs='+',
        required=False,
        help='Trading symbols (e.g., ADAUSD_PERP BTCUSD_PERP). If not specified, downloads all available cm symbols.'
    )
    ...
    if not symbols:
        ...
        temp_downloader = LiquidationSnapshotDownloader(trading_type='cm', max_workers=10)
        symbols = temp_downloader.fetch_symbols('cm')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance COIN-M Futures Daily Liquidation Snapshot Downloader")
    logger.info(f"Market type: cm (COIN-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: DAILY liquidation snapshots")

    return download_liquidation_snapshot_daily(
        symbols=symbols,
