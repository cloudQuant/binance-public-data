#!/usr/bin/env python3
"""
Binance COIN-M Futures Daily Mark Price Klines Downloader

Optimized script for downloading daily mark price klines history from Binance COIN-M Futures.
This script downloads DAILY mark price klines data.
...
Examples:
    ...
    python3 scripts/download-futures-cm-daily-markPriceKlines.py -s BTCUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with custom intervals
    python3 scripts/download-futures-cm-daily-markPriceKlines.py -s BTCUSD_PERP -i 1h 4h -startDate 2023-01-01 -endDate 2023-12-31

    # Download multiple symbols
    python3 scripts/download-futures-cm-daily-markPriceKlines.py -s BTCUSD_PERP ETHUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with auto-detected symbols (all cm symbols)
    python3 scripts/download-futures-cm-daily-markPriceKlines.py -startDate 2023-01-01 -endDate 2023-12-31

    # Specify custom output folder
    python3 scripts/download-futures-cm-daily-markPriceKlines.py -s BTCUSD_PERP -folder /data/binance -startDate 2023-01-01 -endDate 2023-12-31
"""
...
def download_mark_price_klines_daily(
    symbols: list,
    intervals: list,
    start_date: str,
    end_date: str,
    ...
    """
    Download COIN-M Futures daily mark price klines data.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Daily Mark Price Klines Downloader\n'
                    'Downloads DAILY mark price klines data for COIN-M Futures.',
    ...
    if not symbols:
        ...
        temp_downloader = MarkPriceDownloader(trading_type='cm', max_workers=10)
        symbols = temp_downloader.fetch_symbols('cm')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance COIN-M Futures Daily Mark Price Klines Downloader")
    logger.info(f"Market type: cm (COIN-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: DAILY mark price klines")
    logger.info(f"Intervals: {', '.join(args.intervals)}")

    return download_mark_price_klines_daily(
        symbols=symbols,
