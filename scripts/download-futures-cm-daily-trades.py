#!/usr/bin/env python3
"""
Binance COIN-M Futures Daily Trades Downloader

Optimized script for downloading daily trades history from Binance COIN-M Futures.
This script downloads DAILY trades data.
...
Examples:
    ...
    python3 scripts/download-futures-cm-daily-trades.py -s BTCUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download multiple symbols
    python3 scripts/download-futures-cm-daily-trades.py -s BTCUSD_PERP ETHUSD_PERP -startDate 2023-01-01 -endDate 2023-12-31

    # Download with auto-detected symbols (all cm symbols)
    python3 scripts/download-futures-cm-daily-trades.py -startDate 2023-01-01 -endDate 2023-12-31

    # Specify custom output folder
    python3 scripts/download-futures-cm-daily-trades.py -s BTCUSD_PERP -folder /data/binance -startDate 2023-01-01 -endDate 2023-12-31
"""
...
def download_trades_daily(
    ...
    """
    Download COIN-M Futures daily trades data.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Daily Trades Downloader\n'
                    'Downloads DAILY trades data for COIN-M Futures.',
    ...
    if not symbols:
        ...
        temp_downloader = TradeDownloader(trading_type='cm', max_workers=10)
        symbols = temp_downloader.fetch_symbols('cm')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance COIN-M Futures Daily Trades Downloader")
    logger.info(f"Market type: cm (COIN-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: DAILY trades")

    return download_trades_daily(
        symbols=symbols,
