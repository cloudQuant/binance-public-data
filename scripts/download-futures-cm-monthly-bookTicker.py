#!/usr/bin/env python3
"""
Binance COIN-M Futures Book Ticker Downloader (Daily Only)

Downloads daily book ticker (best bid/ask price) history from Binance COIN-M futures.

Note: Book ticker data is only available as DAILY files, not monthly.
...
Examples:
    ...
    python3 scripts/download-futures-cm-monthly-bookTicker.py -s ADAUSD_PERP

    # Download for specific year/month range
    python3 scripts/download-futures-cm-monthly-bookTicker.py -s ADAUSD_PERP -y 2023 2024

    # Download multiple symbols
    python3 scripts/download-futures-cm-monthly-bookTicker.py -s ADAUSD_PERP BCHUSD_PERP

    # Specify custom output folder
    python3 scripts/download-futures-cm-monthly-bookTicker.py -s ADAUSD_PERP -folder /data/binance
"""
...
def download_book_ticker_daily(
    ...
    """
    Download COIN-M futures book ticker daily data.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Book Ticker Downloader (Daily Only)',
    ...
    logger.info("Binance COIN-M Futures Book Ticker Downloader (Daily Only)")
    logger.info(f"Market type: cm (COIN-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: DAILY (book ticker data is only available as daily files)")
