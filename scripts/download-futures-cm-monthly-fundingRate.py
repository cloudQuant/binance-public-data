#!/usr/bin/env python3
"""
Binance COIN-M Futures Funding Rate Downloader (Monthly)

Downloads monthly funding rate history from Binance COIN-M futures.
...
Examples:
    ...
    python3 scripts/download-futures-cm-monthly-fundingRate.py -s ADAUSD_PERP

    # Download for specific year/month range
    python3 scripts/download-futures-cm-monthly-fundingRate.py -s ADAUSD_PERP BTCUSD_PERP -y 2023 2024

    # Download multiple symbols
    python3 scripts/download-futures-cm-monthly-fundingRate.py -s ADAUSD_PERP BCHUSD_PERP

    # Specify custom output folder
    python3 scripts/download-futures-cm-monthly-fundingRate.py -s ADAUSD_PERP -folder /data/binance
"""
...
def download_funding_rate_monthly(
    ...
    """
    Download COIN-M futures funding rate monthly data.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance COIN-M Futures Funding Rate Downloader (Monthly)',
    ...
    if not symbols:
        ...
        temp_downloader = FundingRateDownloader(trading_type='cm', max_workers=10)
        symbols = temp_downloader.fetch_symbols('cm')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance COIN-M Futures Funding Rate Downloader (Monthly)")
