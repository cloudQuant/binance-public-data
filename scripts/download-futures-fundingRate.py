#!/usr/bin/env python3
"""
Binance Futures Funding Rate Downloader (Monthly Only)

Optimized script for downloading funding rate history from Binance futures.
This script downloads MONTHLY funding rate data (daily data is not available for funding rate).
...
from binance_data_downloader.downloaders.funding_rate_downloader import FundingRateDownloader
...
def download_funding_rate_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    years: list = None,
    months: list = None,
    ...
    """
    Download funding rate monthly data with smart features.

    Args:
        trading_type: Market type ('um' or 'cm')
    ...
    try:
        ...
        for symbol in symbols:
            ...
            downloaded_count = downloader.download_monthly(
                symbols=[symbol],
                intervals=None,  # Funding rate doesn't support intervals
                years=years,
                months=months,
                folder=folder,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance Futures Funding Rate Downloader (Monthly Only)\n'
                    'Downloads MONTHLY funding rate data (daily data is not available).',
    ...
    parser.add_argument(
        ...
        default='um',
        choices=['um', 'cm'],
        help='Market type: um (USD-M Futures) or cm (COIN-M Futures). Default: um'
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        required=False,
        help='Trading symbols (e.g., BTCUSDT ETHUSDT). If not specified, downloads all available symbols.'
    )
    ...
    if not symbols:
        logger.info(f"No symbols specified, fetching all {args.type} symbols from exchange...")
        temp_downloader = FundingRateDownloader(trading_type=args.type, max_workers=10)
        symbols = temp_downloader.fetch_symbols(args.type)
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance Futures Funding Rate Downloader (Monthly Only)")
    logger.info(f"Market type: {args.type}")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY (daily data not available for funding rate)")

    return download_funding_rate_monthly(
