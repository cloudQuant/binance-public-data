#!/usr/bin/env python3
"""
Binance USD-M Futures Daily Order Book Depth Downloader

Optimized script for downloading daily order book depth history from Binance USD-M futures.
This script downloads DAILY depth data.
...
Examples:
    ...
    python3 scripts/download-futures-um-daily-bookDepth.py -s BTCUSDT

    # Download for specific date range
    python3 scripts/download-futures-um-daily-bookDepth.py -s BTCUSDT -startDate 2024-01-01 -endDate 2024-12-31

    # Download multiple symbols
    python3 scripts/download-futures-um-daily-bookDepth.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-daily-bookDepth.py -s BTCUSDT -folder /data/binance
"""
...
from binance_data_downloader.downloaders.depth_downloader import DepthDownloader
...
def download_depth_daily(
    ...
    """
    Download order book depth daily data with smart features.
    ...
    try:
        ...
        for symbol in symbols:
            ...
            downloaded_count = downloader.download_daily(
                symbols=[symbol],
                intervals=None,  # Depth doesn't support intervals
                dates=date_list,
                folder=folder,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Daily Order Book Depth Downloader\n'
                    'Downloads DAILY depth data.',
    ...
    if not symbols:
        ...
        temp_downloader = DepthDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Daily Order Book Depth Downloader")
