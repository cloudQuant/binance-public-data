#!/usr/bin/env python3
"""
Binance USD-M Futures Book Ticker Downloader (Monthly Only)

Optimized script for downloading monthly book ticker history from Binance USD-M futures.
Book ticker provides the best bid price, best ask price, and quantities.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY book ticker data only
...
Examples:
    ...
    python3 scripts/download-futures-um-monthly-bookTicker.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-bookTicker.py -s BTCUSDT -y 2023 2024

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-bookTicker.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-bookTicker.py -s BTCUSDT -folder /data/binance
"""
...
def download_book_ticker_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    years: list = None,
    months: list = None,
    ...
    """
    Download book ticker monthly data with smart features.
    ...
    try:
        ...
        downloader = BookTickerDownloader(
        ...
        for symbol in symbols:
            ...
            downloaded_count = downloader.download_monthly(
                symbols=[symbol],
                intervals=None,  # Book ticker doesn't support intervals
                years=years,
                months=months,
                folder=folder,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Book Ticker Downloader (Monthly Only)\n'
                    'Downloads MONTHLY book ticker data for USD-M futures.',
    ...
    if not symbols:
        ...
        temp_downloader = BookTickerDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Book Ticker Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY book ticker")

    return download_book_ticker_monthly(
