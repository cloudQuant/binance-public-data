#!/usr/bin/env python3
"""
Binance USD-M Futures Index Price Klines Downloader (Monthly Only)

Optimized script for downloading monthly index price kline history from Binance USD-M futures.
Index price klines show the index price over time, which is used for mark price calculation.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY index price kline data only
...
- Uses 10 concurrent threads for efficient downloading
- Supports multiple time intervals

Examples:
    # Download with auto-detected date range (recommended)
    python3 scripts/download-futures-um-monthly-indexPriceKlines.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-indexPriceKlines.py -s BTCUSDT -y 2023 2024

    # Download specific intervals
    python3 scripts/download-futures-um-monthly-indexPriceKlines.py -s BTCUSDT -i 1h 4h 1d

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-indexPriceKlines.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-indexPriceKlines.py -s BTCUSDT -folder /data/binance
"""
...
def download_index_price_klines_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    intervals: list = None,
    years: list = None,
    ...
    """
    Download index price klines monthly data with smart features.

    Args:
        trading_type: Market type (hardcoded to 'um' for USD-M Futures)
        symbols: List of trading symbols
        intervals: List of time intervals (None for default: ['1m', '15m', '1h', '4h', '8h'])
    ...
    try:
        ...
        downloader = IndexPriceDownloader(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Index Price Klines Downloader (Monthly Only)\n'
                    'Downloads MONTHLY index price kline data for USD-M futures.',
    ...
    if not symbols:
        ...
        temp_downloader = IndexPriceDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Index Price Klines Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY index price klines")

    return download_index_price_klines_monthly(
        trading_type='um',  # Hardcoded for USD-M Futures
        symbols=symbols,
