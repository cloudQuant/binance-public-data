#!/usr/bin/env python3
"""
Binance USD-M Futures Klines Downloader (Monthly Only)

Optimized script for downloading monthly kline (candlestick) history from Binance USD-M futures.
Klines show open, high, low, close prices and volume over time.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY kline data only
...
Examples:
    ...
    python3 scripts/download-futures-um-monthly-klines.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-klines.py -s BTCUSDT -y 2023 2024

    # Download specific intervals
    python3 scripts/download-futures-um-monthly-klines.py -s BTCUSDT -i 1h 4h 1d

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-klines.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-klines.py -s BTCUSDT -folder /data/binance
"""
...
def download_klines_monthly(
    ...
    """
    Download klines monthly data with smart features.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Klines Downloader (Monthly Only)\n'
                    'Downloads MONTHLY kline (candlestick) data for USD-M futures.',
    ...
    if not symbols:
        ...
        temp_downloader = KlineDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Klines Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY klines (candlesticks)")
