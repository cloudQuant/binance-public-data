#!/usr/bin/env python3
"""
Binance USD-M Futures Mark Price Klines Downloader (Monthly Only)

Optimized script for downloading monthly mark price kline history from Binance USD-M futures.
Mark price is used for liquidation calculations and unrealized PnL.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY mark price kline data only
...
Examples:
    ...
    python3 scripts/download-futures-um-monthly-markPriceKlines.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-markPriceKlines.py -s BTCUSDT -y 2023 2024

    # Download specific intervals
    python3 scripts/download-futures-um-monthly-markPriceKlines.py -s BTCUSDT -i 1h 4h 1d

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-markPriceKlines.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-markPriceKlines.py -s BTCUSDT -folder /data/binance
"""
...
def download_mark_price_klines_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    intervals: list = None,
    years: list = None,
    ...
    """
    Download mark price klines monthly data with smart features.
    ...
    try:
        ...
        downloader = MarkPriceDownloader(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Mark Price Klines Downloader (Monthly Only)\n'
                    'Downloads MONTHLY mark price kline data for USD-M futures.',
    ...
    if not symbols:
        ...
        temp_downloader = MarkPriceDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Mark Price Klines Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY mark price klines")
