#!/usr/bin/env python3
"""
Binance USD-M Futures Premium Index Klines Downloader (Monthly Only)

Optimized script for downloading monthly premium index kline history from Binance USD-M futures.
Premium index shows the premium/discount of futures price relative to index price.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY premium index kline data only
...
Examples:
    ...
    python3 scripts/download-futures-um-monthly-premiumIndexKlines.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-premiumIndexKlines.py -s BTCUSDT -y 2023 2024

    # Download specific intervals
    python3 scripts/download-futures-um-monthly-premiumIndexKlines.py -s BTCUSDT -i 1h 4h 1d

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-premiumIndexKlines.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-premiumIndexKlines.py -s BTCUSDT -folder /data/binance
"""
...
def download_premium_index_klines_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    intervals: list = None,
    years: list = None,
    ...
    """
    Download premium index klines monthly data with smart features.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Premium Index Klines Downloader (Monthly Only)\n'
                    'Downloads MONTHLY premium index kline data for USD-M futures.',
    ...
    logger.info("Binance USD-M Futures Premium Index Klines Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY premium index klines")

    return download_premium_index_klines_monthly(
