#!/usr/bin/env python3
"""
Binance USD-M Futures Trades Downloader (Monthly Only)

Optimized script for downloading monthly trade history from Binance USD-M futures.

Features:
- Hardcoded for USD-M Futures (um) market type
- Downloads MONTHLY trade data only
...
Examples:
    ...
    python3 scripts/download-futures-um-monthly-trades.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-futures-um-monthly-trades.py -s BTCUSDT -y 2023 2024

    # Download multiple symbols
    python3 scripts/download-futures-um-monthly-trades.py -s BTCUSDT ETHUSDT

    # Specify custom output folder
    python3 scripts/download-futures-um-monthly-trades.py -s BTCUSDT -folder /data/binance
"""
...
from binance_data_downloader.downloaders.trade_downloader import TradeDownloader
from binance_data_downloader.utils.logger_setup import setup_logger, log_level_from_string


def download_trades_monthly(
    trading_type: str = 'um',
    symbols: list = None,
    years: list = None,
    months: list = None,
    ...
    """
    Download trades monthly data with smart features.
    ...
    try:
        ...
        for symbol in symbols:
            ...
            downloaded_count = downloader.download_monthly(
                symbols=[symbol],
                intervals=None,  # Trades don't support intervals
                years=years,
                months=months,
                folder=folder,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance USD-M Futures Trades Downloader (Monthly Only)\n'
                    'Downloads MONTHLY trade data for USD-M futures.',
    ...
    if not symbols:
        ...
        temp_downloader = TradeDownloader(trading_type='um', max_workers=10)
        symbols = temp_downloader.fetch_symbols('um')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance USD-M Futures Trades Downloader (Monthly Only)")
    logger.info(f"Market type: um (USD-M Futures)")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY trades")

    return download_trades_monthly(
