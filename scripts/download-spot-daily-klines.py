#!/usr/bin/env python3
"""
Binance Spot Daily Klines Downloader

Optimized script for downloading daily kline (candlestick) data from Binance spot market.

Features:
- Downloads DAILY kline data for spot market
- Supports multiple time intervals (1m, 15m, 1h, 4h, 8h, etc.)
- Auto-skips existing files
- Shows detailed progress (symbol, date, interval, file size)

Examples:
    # Download with default date range (2020-01-01 to current) and default intervals
    python3 scripts/download-spot-daily-klines.py -s BTCUSDT

    # Download for specific date range
    python3 scripts/download-spot-daily-klines.py -s BTCUSDT -startDate 2023-01-01 -endDate 2023-12-31

    # Download specific intervals
    python3 scripts/download-spot-daily-klines.py -s BTCUSDT -i 1m 1h 1d

    # Download multiple symbols
    python3 scripts/download-spot-daily-klines.py -s BTCUSDT ETHUSDT

    # Download all spot symbols with default intervals
    python3 scripts/download-spot-daily-klines.py

    # Specify custom output folder
    python3 scripts/download-spot-daily-klines.py -s BTCUSDT -folder /data/binance
"""
...
# Default intervals for daily klines
DEFAULT_INTERVALS = ["1m", "15m", "1h", "4h", "8h"]
...
def download_klines_daily(
    ...
    """
    Download daily kline data for spot market.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance Spot Daily Klines Downloader\n'
                    'Downloads DAILY kline (candlestick) data for spot market.',
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help=f'Kline intervals (e.g., 1m 1h 1d). Default: {", ".join(DEFAULT_INTERVALS)}'
    )
    ...
    if not symbols:
        ...
        temp_downloader = KlineDownloader(trading_type='spot', max_workers=10)
        symbols = temp_downloader.fetch_symbols('spot')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance Spot Daily Klines Downloader")
    logger.info(f"Market type: spot")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Intervals: {', '.join(args.intervals if args.intervals else DEFAULT_INTERVALS)}")
