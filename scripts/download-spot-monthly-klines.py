#!/usr/bin/env python3
"""
Binance Spot Klines Downloader (Monthly)

Downloads monthly kline data from Binance spot market.
...
Examples:
    # Download with default intervals
...
VALID_INTERVALS = ["1m", "15m", "1h", "4h", "8h"]
...
def download_klines_monthly(
    ...
    try:
        ...
        if not intervals:
            ...
            logger.info(f"No intervals specified, using default: {', '.join(intervals)}")
        ...
        for symbol in symbols:
            ...
            # Download monthly data for each interval
            for interval in intervals:
                ...
                if downloaded_count > 0:
                    logger.info(f"Successfully downloaded {downloaded_count} files for {symbol} ({interval})")
                else:
                    logger.info(f"All files already exist for {symbol} ({interval})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance Spot Klines Downloader (Monthly)',
    ...
    parser.add_argument(
        ...
        default=None,
        help='Kline intervals (default: 1m 15m 1h 4h 8h). Example: -i 1m 1h'
    )
    ...
    logger.info("Binance Spot Klines Downloader (Monthly)")
