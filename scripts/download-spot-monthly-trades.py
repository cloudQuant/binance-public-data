#!/usr/bin/env python3
"""
Binance Spot Trades Downloader (Monthly Only)

Optimized script for downloading individual trade history from Binance spot market.
This script downloads MONTHLY trade data.
...
Examples:
    ...
    python3 scripts/download-spot-monthly-trades.py -s BTCUSDT

    # Download for specific year/month range
    python3 scripts/download-spot-monthly-trades.py -s BTCUSDT -y 2023 2024

    # Download multiple symbols
    python3 scripts/download-spot-monthly-trades.py -s BTCUSDT ETHUSDT

    # Download for specific months
    python3 scripts/download-spot-monthly-trades.py -s BTCUSDT -y 2024 -m 1 2 3

    # Specify custom output folder
    python3 scripts/download-spot-monthly-trades.py -s BTCUSDT -folder /data/binance
"""
...
def download_trades_monthly(
    ...
    """
    Download spot trade monthly data with smart features.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Binance Spot Trades Downloader (Monthly Only)\n'
                    'Downloads MONTHLY individual trade data.',
    ...
    if not symbols:
        ...
        temp_downloader = TradeDownloader(trading_type='spot', max_workers=10)
        symbols = temp_downloader.fetch_symbols('spot')
        logger.info(f"Found {len(symbols)} symbols")

    logger.info("Binance Spot Trades Downloader (Monthly Only)")
    logger.info(f"Market type: spot")
    logger.info(f"Symbols: {', '.join(symbols[:10])}{'...' if len(symbols) > 10 else ''}")
    logger.info(f"Data type: MONTHLY individual trades")
